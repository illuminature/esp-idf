...
esp_err_t esp_efuse_utility_burn_chip(void)
{
    ...
    if (esp_efuse_set_timing() != ESP_OK) {
        ESP_EARLY_LOGE(TAG, "Efuse fields are not burnt");
    } else {
        ...
        for (int num_block = EFUSE_BLK_MAX - 1; num_block >= EFUSE_BLK0; num_block--) {
            ...
            if (error) {
                ...
                ESP_EARLY_LOGE(TAG, "BLOCK%d can not be burned because a previous block got an error, skipped.", num_block);
                continue;
            }
            ets_efuse_clear_program_registers();
            ...
            if (coding_error_before) {
                ESP_EARLY_LOGW(TAG, "BLOCK%d already has a coding error", num_block);
            }
            bool coding_error_occurred;

            do {
                ESP_EARLY_LOGI(TAG, "BURN BLOCK%d", num_block);
                ...
                if (coding_error_occurred) {
                    ESP_EARLY_LOGW(TAG, "BLOCK%d got a coding error", num_block);
                }
                ...
                if (!correct_written_data || coding_error_occurred) {
                    ESP_EARLY_LOGW(TAG, "BLOCK%d: next retry to fix an error [%d/3]...", num_block, repeat_burn_op);
            ...
            if (coding_error_occurred) {
                ESP_EARLY_LOGW(TAG, "Coding error was not fixed");
                if (num_block == 0) {
                    ESP_EARLY_LOGE(TAG, "BLOCK0 got a coding error, which might be critical for security");
                    error = ESP_FAIL;
                }
            }
            if (!correct_written_data) {
                ESP_EARLY_LOGE(TAG, "Written data are incorrect");
                error = ESP_FAIL;
            }
        }
    }
